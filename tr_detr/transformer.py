...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_queries=2, num_encoder_layers=6,
        ...
        # self.saliency_proj2 = nn.Linear(d_model, d_model) global
    ...
    def forward(self, src, mask, query_embed, pos_embed, saliency_proj1, video_length=None): # 和主函数权重共享
        """
        Args:
            src: (batch_size, L, d)
            mask: (batch_size, L)
        ...
        return hs, references, memory_local, saliency_scores
    
    def HD2MR(self, memory_local, saliency_proj1, src, video_length, mask_local, pos_embed_local):

        vid_mem = memory_local.transpose(0, 1)
        saliency_scores = (torch.sum(saliency_proj1(vid_mem), dim=-1) / np.sqrt(self.d_model))
        weight = torch.softmax(saliency_scores, dim=-1) # ()
        src_vid = src[:video_length]
        src_vid_wg = src_vid * weight.transpose(1, 0).unsqueeze(-1)
        memory_local = memory_local + src_vid_wg 
        memory_local = self.encoder(memory_local, src_key_padding_mask=mask_local, pos=pos_embed_local) 
        return memory_local, saliency_scores
...
class T2V_TransformerEncoderLayer_no_global(nn.Module):
    ...
    def forward_pre(self, src,
        ...
        q, k, v = pos_src[1:76], pos_src[76:], src2[76:]
        # print(q.shape) # 100 32 256


        src2 = self.self_attn(q, k, value=v, attn_mask=src_key_padding_mask[:, :75].permute(1,0),
                              key_padding_mask=src_key_padding_mask[:, 75:])[0]
        src2 = src[:75] + self.dropout1(src2)
        src3 = self.norm1(src2)
        src3 = self.linear2(self.dropout(self.activation(self.linear1(src3))))
        src2 = src2 + self.dropout2(src3)
        src2 = self.norm2(src2)
